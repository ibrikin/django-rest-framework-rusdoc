...
> View на основе джанго есть отсылка к старому views
>
> &mdash; [Reinout van Rees][cite]

REST framework provides an `APIView` class, which subclasses Django's `View` class.
REST framework предоставляет класс `APIView` который является подклассом Джанго класса `View`

`APIView` classes are different from regular `View` classes in the following ways:
`APIView` класс - отличается от `View` класса следующим:

* Requests passed to the handler methods will be REST framework's `Request` instances, not Django's `HttpRequest` instances.
* Запросы пришедшие к обработчикам методов будут экземплярами REST framework's `Request`, а не экземпляром Джанго `HttpRequest`.
* Handler methods may return REST framework's `Response`, instead of Django's `HttpResponse`.  The view will manage content negotiation and setting the correct renderer on the response.
* Методы обработчика могут вернуть `Response`, вместо `HttpResponse`. View будет управлять взаимодействием контента, и устанавливать правильные обработчик для ответа.
* Any `APIException` exceptions will be caught and mediated into appropriate responses.
* Любое исключение `APIException` будут словлены и будут привязаны к определенным ответам.
* Incoming requests will be authenticated and appropriate permission and/or throttle checks will be run before dispatching the request to the handler method.
* Входящие запросы будут авторизовываться и соответсвовать разрешению и/или запрещающим проверкам которые будут запускаться до выпуска запроса с методом обработки
...
Использование класса `APIView` очень похоже на использование класса `View`, как обычно, входящий запрос отправляется к определенному обработчику метода как `.get()` или `.post()`. Дополнительно, количество атрибутов может быть установлено в класс который контролирует некоторые аспекты API прав. 
